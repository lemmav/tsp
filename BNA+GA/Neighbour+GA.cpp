...
void ConvertWayForGA(Way& VisitedPoint) { 
	Way tempWay; int tempi;
	if (VisitedPoint[0] == 0) VisitedPoint.erase(VisitedPoint.begin());
	else if (VisitedPoint[VisitedPoint.size() - 1] == 0) VisitedPoint.pop_back();
	else {
		for (int i = 0; i < VisitedPoint.size(); i++) 
			if (VisitedPoint[i] == 0) { tempi = i; break; }
		tempWay.insert(tempWay.end(), VisitedPoint.begin() + tempi+1, VisitedPoint.end());
		tempWay.insert(tempWay.end(), VisitedPoint.begin(), VisitedPoint.begin() + tempi);
		VisitedPoint = tempWay;
	}
	
}
Generation GenerationGeneratorFromNeighbour(vector<Way> Visited, int creaturesnumber) {
	Generation generation;
	for (int i = 0; i < Visited.size(); i++) {
		ConvertWayForGA(Visited[i]);
		Creature creature; creature.way = Visited[i];
		generation.push_back(creature);
	}
	for (int i = 0; i < creaturesnumber - Visited.size(); i++) {
		Creature creature;
		WayShuffling(creature);
		generation.push_back(creature);
	}
...
int BNNSNearestWayFinder(Graph graph, int index, double& wayLength, Visit& visit) {
	...
	for (int i = 0; i < N; i++)
		...
		if (index!=i && graph[index][i] < minimalWay && visit[i] == false ) {
...
double BNNSTravelingSalesmanNeighbour(Graph& graph, Visit& visit, int i, Way& Visited) {
...
vector< Way> BNNSFinder(Graph& graph, Visit& visit) {
...
void NGA(double mutation,  int creaturesnumber) {
	...
	vector< Way> allWay = BNNSFinder(graph, visit);

	Generation generation = GenerationGeneratorFromNeighbour(allWay, creaturesnumber);
	...
	while (count <= 100) {
		...
		while (newGeneration.size() < generation.size()) {
			Creature parent1 = TournamentSelection(generation, generation.size());
			Creature parent2 = TournamentSelection(generation, generation.size());
